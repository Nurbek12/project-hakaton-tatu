            ...
            <v-data-table-server
              ...
              <template #item.status="{ index, item }">
                <td :data-label="t('posts.status')">
                  <v-chip variant="flat" label :color="item.is_published?'green':'orange'" class="text-white">{{ t(item.is_published?"posts.published":"posts.not_published") }}</v-chip>
                </td>
              </template>
              <template #bottom>
...
import { get_posts, delete_post } from '../../request/posts'
...
const deleteItem = async (id, i) => {
  if(!confirm('Вы хотите удалить этот пост?')) return
...
const loadItems =  async ({page, itemsPerPage }) => {
  loading.value = true
  const { data } = await get_posts(`limit=${itemsPerPage}&offset=${(page - 1) * parseInt(itemsPerPage)}&creator=${getters.user?.username||''}`)
