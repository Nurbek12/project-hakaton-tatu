<template>
    <v-navigation-drawer v-model="drawer" :rail="rail" permanent width="250" border :expand-on-hover="mobile">
      <v-list :lines="false" nav mandatory variant="flat">
        <v-list-item v-for="(item, i) in links[store.getters.user?.role||'']" link :to="item.url" :key="i" :value="item" color="primary" exact>
      ...
      <v-btn border position="absolute" v-if="!mobile" :icon="`mdi-chevron-${!rail ? 'left' : 'right'}`" size="x-small" @click="rail = !rail" style="right: -15px"></v-btn>
